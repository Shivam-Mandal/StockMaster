...
const getAllProducts = async (req, res) => {
  try {
    ...
    const products = await Product.find({ store: storeId }).populate("supplier", "contactPerson");;
...
const addProducts = async (req, res) => {
  try {
    ...
    const existingProduct = await Product.findOne({ name, store });

    let totalQuantity = quantityInStock || 0;
    let reorderLevelToUse = reorderLevel || 50;

    if (existingProduct) {
      totalQuantity += existingProduct.quantityInStock;

      if (!reorderLevel) {
        reorderLevelToUse = existingProduct.reorderLevel;
      }
    }
    ...
    if (totalQuantity === 0) {
      status = 'out-of-stock';
    } else if (totalQuantity <= reorderLevelToUse) {
      status = 'low-stock';
    }
    const formattedCostPrice = parseFloat(costPrice).toFixed(2);
    const formattedSellingPrice = parseFloat(sellingPrice).toFixed(2);
